...
async function buyApi(tx_id) {
  ...
  const resp = await $http.post(
    "https://api.lionnft.io/v1/buy/tx_id",
    formData
  );
  return resp;
}
...
async function bidCreateApi(order) {
  const resp = await $http.post(
    "https://api.lionnft.io/v1/item/bids/create",
    order
  );
  return resp;
}
...
async function bidOrderFeeApi(order) {
  const resp = await $http.post(
    "https://api.lionnft.io/v1/chain/prepare/ordermessage",
    order
  );
  return resp;
}
...
async function bidTxidApi(tx_id) {
  ...
  formData.append("tx_id", tx_id);
  const resp = await $http.post(
    "https://api.lionnft.io/v1/bids/tx_id",
    formData
  );
  return resp;
}
